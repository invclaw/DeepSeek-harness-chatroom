...
`,s=X(e,S);continue}if(nr(e,s,S)){var T=_r(e,s,S,c);if(gr>0||T<S){for(var E=``,ee=0;ee<gr;ee++)E+=` `;l+=`${E+e.slice(T,S)}
`}else l+=`
`;s=X(e,S);for(var D=!1,te=0;te<l.length;te++){var O=l.charCodeAt(te);if(O!==B&&O!==Ce&&O!==R&&O!==z){D=!0;break}}if(u&&!D){if(s<e.length){var ne=Xn(e,s),re=hr(e,s,ne);if(!re||re.ordered!==a.ordered||re.marker!==a.marker)break;d=!0}else break}if(s<e.length){var ie=Xn(e,s);tr(e,s,ie);var ae=e.charCodeAt(s+Z);if((ae===U||ae===je||ae===Me)&&er<=3&&or(e,s))break;var oe=hr(e,s,ie);if(oe&&oe.ordered===a.ordered&&oe.marker===a.marker&&oe.markerCol<c){d=!0;continue}if(!nr(e,s,ie)&&er<c)break}continue}if(er>=c){var se=_r(e,s,S,c);if(gr>0){for(var ce=``,le=c,ue=0;ue<gr;ue++)ce+=` `,le++;for(var k=se;k<S;k++)if(e.charCodeAt(k)===z){for(var A=4-le%4,j=0;j<A;j++)ce+=` `;le+=A}else ce+=e[k],le++;l+=`${ce}
`}else l+=`${e.slice(se,S)}
`;s=X(e,S);continue}for(var M=!1,de=0;de<l.length;de++){var fe=l.charCodeAt(de);if(fe!==B&&fe!==Ce&&fe!==R&&fe!==z){M=!0;break}}if(!d&&M&&!u){var N=s+Z,P=e.charCodeAt(N);if(!(P===De||P===H||P===Ne||P===V||P===we||(P===U||P===je||P===Me||P===tt)&&(or(e,s)!=null||hr(e,s,S)!=null)||P>=W&&P<=G&&hr(e,s,S)!=null)){l+=`${e.slice(N,S)}
...
`)!==-1||Cn.test(Ze);ct&&!lt&&(st=!0)}if(!me&&(n.inHTML||c===7||Ie||!Le||st)){var ut,dt,ft=!1;if(P!==-1&&Re){var pt=!1;if(n.inHTML&&pe<N.length){for(var W=pe;W<N.length&&N.charCodeAt(W)!==B;)W++;var G=N.slice(pe,W).trim();pt=G.length>1&&G.charCodeAt(0)===Ne&&G.charCodeAt(1)!==Qe}dt=pt?N.slice(A.end-a):Ze,ft=!0}else(c===7||n.inHTML)&&P!==-1?(dt=N.slice(A.end-a),dt.charCodeAt(0)===B&&(dt=dt.slice(1))):Re?ut=e.slice(a,ze):Ie?ut=ue:(dt=N.slice(A.end-a),dt.charCodeAt(0)===B&&(dt=dt.slice(1)));var J=ut!==void 0,mt=Rn(ut===void 0?dt||``:ut),ht={type:Y.htmlBlock,tag:j,attrs:br(A.attrs,j,r),b:A.g+A.b,children:Xe,e:J?void 0:mt,j:J?mt:void 0,text:mt,c:!0,a:!1};return ft&&(ht.x=!0),{node:ht,end:Be}}var gt=Rn(Ze);return{node:{type:Y.htmlBlock,tag:j,attrs:br(A.attrs,j,r),b:A.g+A.b,children:Xe,e:me?void 0:gt,text:gt,c:!1,a:!1},end:Be}}var _t=ue.match(/^<(\/?)([a-zA-Z][a-zA-Z0-9-]*)/),vt=_t?_t[2]:`div`,yt=_t?_t[1]===`/`:!1,bt,xt,St,Ct=``;if(yt){var wt=ue.indexOf(`>`);wt===-1?bt=ue:(bt=ue.slice(0,wt+1),Ct=ue.slice(wt+1),Ct&&(xt=Rn(Ct)))}else St=Rn(ue);return{node:{type:Y.htmlBlock,tag:vt,attrs:{},children:[],e:xt,h:bt,j:St,text:yt?Ct:ue,c:!0,a:yt},end:k}}var Tt=Ln(e,a);if(!Tt)return null;var Et=Tt.tag,Dt=Et.toLowerCase(),Ot=Et.charCodeAt(0),kt=Ot>=K&&Ot<=q;if(!(kt||yr.has(Dt)||bn.has(Dt)||Dt.includes(`-`)))return null;if(Tt.f)return{node:{type:Y.htmlSelfClosing,tag:Et,attrs:{},h:e.slice(a,Tt.end),a:!0},end:Tt.end};var At=wr(e,Tt.end,Et),jt=[];if(At!==-1){var Nt=Cr,Pt=e.slice(Tt.end,Nt),Ft=Pt.trim();if(Ft){var It=Pt.indexOf(`
