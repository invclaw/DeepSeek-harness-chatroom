...
- Persistent branches open in a right-side panel and own independent Harness Sessions. Every regular branch prompt enters that branch Agent immediately, without requiring `@AI`; the parent room remains human-first. Branches support Markdown, `@` candidates, images/files, quotes, reactions, forwarding, selection, and the full Agent runtime without nested chatroom branches. Their sidebar rows use a compact marker and parent context instead of a heavy edge stripe, keeping the hierarchy visible without making the branch look like a second room. Each room keeps its two most recently updated branches visible and provides an independent expand/collapse control for older branches. Gateways that reject embedded documents switch immediately to an inline compatibility view instead of waiting for a timeout, with the complete Agent available in a new tab.
...
- **Quick meeting** in Group and Direct resolves the initiating platform account through Enterprise WeChat contacts, adds that person as an attendee, creates a default 60-minute online meeting with the shared authorization, and posts it immediately; Solo intentionally omits the action. Agents can manage further attendees through the official meeting operations. Meeting cards refresh from `init` to `started` and `end`. After a Group meeting ends, the configured meeting-summary model summarizes verified meeting metadata and available official notes, then posts the result back into that Group. Meeting and document results render as native cards with titles, times, attendees, owners, status, and links instead of plain text.
...
10. Choose **Branch** from a human or AI message's overflow menu. The panel must contain the complete native Harness conversation. Verify model and permission selection, image/file upload, stop/steer, slash commands, approval/question interactions, Think/tool trajectory, and retry UI. An ordinary prompt such as `Hello` must start that branch Agent without `@AI`; typing `@` still lists AI and room members, and every Agent output stays in the branch. Reply, react, forward, and multi-select a branch message; no nested chatroom branch action may appear. After four messages, close the panel and verify the root's total and latest three replies.
