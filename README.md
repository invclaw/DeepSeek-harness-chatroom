...
- Timestamps, compact in-composer reply quotes, an expanded common-emoji picker, message reactions, Markdown, image previews, and authenticated file upload/download. Text-only messages bypass attachment preparation, appear immediately, and clear the settled draft.
...
- **1.4.0** — replace per-user Enterprise WeChat authorization with one deployment-wide account managed in Settings; add disconnect/rebind controls, meeting lifecycle status, configurable AI summaries, and authenticated summary APIs; make direct-open branch Sessions reuse room message/composer components; validate branch-root capability coordinates; and apply the parent room's human-first automatic-response controller to branch messages.
