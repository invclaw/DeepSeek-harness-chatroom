...
0.7.3 为分支输入框补齐 AI/成员提及菜单，并让文本模型安全跳过群聊和分支历史中的图片。图片仍保留在原生会话和聊天室界面中；替换仅发生在聊天室插件拥有的模型请求上，不影响其他 Harness Session。原生消息增强层会等待 Harness 消息渲染器就绪，因此卸载重装改变插件顺序后仍能复用原生气泡界面。
