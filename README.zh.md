...
0.6.1 建立线上协作群聊的基础：持久成员、群管理、跨房间未读提醒和持久分支会话。分支在 Harness 原生会话右侧打开，普通分支消息仍是人类聊天，只有明确 `@AI` 才会唤醒该分支自己的 Agent；AI 回答只出现在分支中。主群继续完整使用 Harness 原生对话界面，直接进入群聊的访客在选择身份和头像期间也只会看到干净的成员气泡，不会暴露内部同步标记。
