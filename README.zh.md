...
0.5.1 在 0.4.0 的多会话、人类优先模式上增加头像身份、原生 `@AI` 候选、回复引用和文件卡片。每个浏览器只把自己的消息放在右侧，其他参与者和 AI 在左侧；真人消息显示头像和气泡上方的昵称。图片会按 `maxImageSidePixels` 和 Harness 的总像素限制自动缩小。更换身份只在提交时生效，关闭选择器会保留当前参与者和房间。
