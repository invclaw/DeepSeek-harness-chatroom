...
0.3.0 移除了旧版自绘全屏聊天页和“AI 可静默”提示。聊天室现在只是原生共享 Session 的身份与导航层；AI 回复行为与普通 Harness 会话一致。每个浏览器只把自己的消息放在右侧，其他参与者和 AI 的消息通过 Harness 原生消息组件显示在左侧。原生共享 Session 已选中时，入口弹窗会立即关闭；身份弹窗关闭后不会自行重现，需要时可从原生会话头重新打开。
