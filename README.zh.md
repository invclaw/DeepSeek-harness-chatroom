...
- 群管理抽屉提供邀请链接、成员头像、在线状态和最近活跃时间；群主可修改群名、设置或取消管理员，管理员也可修改群名
...
1.1.2 将“设置 → 群聊与账号”重做为不会横向溢出的原生设置单列，全面使用 Harness 主题令牌、带标签的账号与 OIDC 字段、响应式成员操作，以及与模型和 Agent 预设页面一致的模块底色、细边框和胶囊控件。1.1.1 不再写入用于初始化的伪造轮次，第一次 AI 运行完全沿用 Harness 原生轮次编号。1.1.0 让 Harness 新会话默认成为共享群聊，移除右下角启动按钮，把账号、注册策略和 SSO 收敛到原生“设置 → 群聊与账号”，并将群管理改为带邀请链接的右侧抽屉；部署 dsh-auth 时默认优先使用它登录，`local=1` 保留本地账号应急入口。1.0.1 新增默认 SSO 自动跳转。
...
`sessionId` 是升级前大厅继续使用的持久 Session。普通 Harness Session 第一次由已登录成员打开时，插件会按 Session ID 幂等建立共享群记录，不创建第二套会话；邀请链接把成员直接带到同一个原生 Session。每个分支仍获得独立持久 Session。聊天室文件、成员、表情贴附、分支元数据、分支消息和分支引用保存在同一个 `chatroom` storage domain。
...
9. 在会话头打开“群管理”；右侧抽屉应显示邀请链接、成员头像和在线状态。复制邀请链接让 Bob 进入同一 Session，群主修改群名、将 Bob 设为管理员，再由 Bob 修改群名；刷新后名称和角色应保持。
