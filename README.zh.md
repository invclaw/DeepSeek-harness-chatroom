...
0.7.1 让分支和批量选择在主对话中可见。分支新消息会实时投影到根消息旁，只显示最近 3 条简要内容和总回复数；进入多选状态后，所有可转发的真人与 AI 消息会统一显示左侧复选框，选中 0 条时也保持多选状态，直到用户主动取消。
...
10. 在真人或 AI 消息下点击“分支”，右侧应打开分栏。连续发送 4 条分支消息，关闭分栏后根消息下方应显示总回复数和最近 3 条；再发送 `@AI 总结`，AI 回答只能出现在当前分支并更新该摘要，不能进入主群。
