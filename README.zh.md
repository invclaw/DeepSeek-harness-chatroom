...
1.1.1 不再写入用于初始化的伪造轮次，第一次 AI 运行完全沿用 Harness 原生轮次编号。1.1.0 让 Harness 新会话默认成为共享群聊，移除右下角启动按钮，把账号、注册策略和 SSO 收敛到原生“设置 → 群聊与账号”，并将群管理改为带邀请链接的右侧抽屉；部署 dsh-auth 时默认优先使用它登录，`local=1` 保留本地账号应急入口。1.0.1 新增默认 SSO 自动跳转。
