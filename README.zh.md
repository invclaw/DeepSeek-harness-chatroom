...
0.8.0 将分支面板加宽到桌面端 720px，并让分支消息复用主群的消息操作组件；AI 分支回复支持安全 GFM Markdown，分支内可复制、引用回复、贴表情、转发和多选，但不会出现二级分支入口。引用元数据和模型可读上下文都会持久保存。0.7.5 的远程模型配置白名单通道、分支提及和文本模型图片历史兼容继续保留。
