...
0.8.2 让主群和分支消息复用同一个能力驱动的常驻操作栏；复制、回复、快捷点赞、完整表情选择、转发和多选在两处都直接可见，分支内只关闭继续发起分支的能力。0.8.1 的桌面端 720px 分栏、安全的纯浏览器 GFM 渲染器、浏览器包门禁、持久引用元数据和模型可读引用上下文继续保留。0.7.5 的远程模型配置白名单通道、分支提及和文本模型图片历史兼容也继续保留。
