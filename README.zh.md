...
- 支持在加宽的右侧分栏发起持久分支回复；AI 回复支持安全 GFM Markdown，分支与主群复用同一套头像、气泡、表情贴附、复制、引用、转发和多选组件，但分支内不允许继续嵌套分支
- 分支输入框提供 AI 和群成员的 `@` 候选，每个分支使用独立 Harness Session，分支内 `@AI` 的回答不会进入主群，引用回复也会持久进入该分支上下文
...
0.8.1 将分支面板加宽到桌面端 720px，并让分支消息复用主群的消息操作组件；AI 分支回复通过纯浏览器渲染器支持安全 GFM Markdown，构建会拒绝 Harness 浏览器加载器无法提供的 Node 内建模块。分支内可复制、引用回复、贴表情、转发和多选，但不会出现二级分支入口。引用元数据和模型可读上下文都会持久保存。0.7.5 的远程模型配置白名单通道、分支提及和文本模型图片历史兼容继续保留。
...
`sessionId` 是升级前大厅继续使用的持久 Session。通过界面新建的共享会话会获得独立 Session 和上下文，每个分支也会获得独立持久 Session。聊天室文件、成员、表情贴附、分支元数据、分支消息和分支引用保存在同一个 `chatroom` storage domain，下载端点需要有效聊天室 Cookie。合并转发卡片作为原生 Session 消息持久化。新增可选字段不改变 domain 版本，已有身份、大厅和分支数据可直接读取。
...
10. 在真人或 AI 消息下点击“分支”，右侧应打开加宽分栏。输入 `@` 应看到 AI 和当前群成员；发送要求包含粗体、列表和代码的 `@AI` 请求，回答应按 Markdown 渲染。对任意分支回复执行复制、引用、贴表情、转发和多选；引用卡片应出现在编辑器及发送后的消息中，且分支回复上不能出现继续发起分支的入口。连续发送 4 条消息后关闭分栏，根消息下方应显示总回复数和最近 3 条，AI 回答不能进入主群。
