...
- 群管理面板持久记录成员，显示成员头像、在线状态和最近活跃时间；群主可修改群名、设置或取消管理员，管理员也可修改群名
- 支持页内新消息提示、网页标题未读数，以及由用户主动开启的浏览器系统通知
- 支持在右侧分栏发起持久分支回复；分栏直接装载该分支的完整 Harness 原生会话界面，原生 Markdown、图片/文件、模型选择、权限模式、停止/排队/转向、斜杠命令、审批、问答交互、Think/工具轨迹、失败详情和重试全部可用
...
0.9.7 改为通过精确 Session ID 和原生输入框确认分支就绪，不再比较会被 Harness 再次截短的长标题，因此基于较长 AI 回复创建的分支不会在实际加载成功后被误报为超时。关闭分支时会将最近的原生 Harness 运行时保留在页面中，重复打开不再重新冷启动整套 Web 客户端。0.9.6 将分支从自制消息面板替换为隔离的 Harness 原生 Session，并加入群主/管理员管理、来源事件校验后的无损合并转发和精简消息操作栏。
...
9. 在会话头打开“群管理”；两名参与者应显示正确头像和在线状态。群主修改群名、将 Bob 设为管理员，再由 Bob 修改群名；刷新后名称和角色应保持。由此处主动开启系统通知，隐藏网页后让另一人发消息，应出现系统通知和网页标题未读数。
