...
- 常驻时间戳、输入框内的紧凑回复引用、扩充后的常用表情面板、消息贴表情、Markdown、图片预览以及经过认证的文件上传下载；纯文字消息跳过附件准备，立即显示并清空已经成功发送的草稿。
...
- 群聊、分支和私聊输入框提供“快速会议”：先从企微通讯录解析当前平台发起人，把发起人加入参会人，再用共享企微授权创建默认 60 分钟在线会议并立即发到当前会话；Solo 不显示该入口。Agent 还可以通过官方会议操作继续管理参会人。会议卡片会从未开始、进行中自动更新到已结束；群聊或分支会议结束后，由设置页选择的总结模型根据可信会议元数据和官方纪要生成总结，并以可复制、引用、转发和多选的标准 AI 消息发回原会话。会议与文档结果以原生卡片展示标题、时间、参与人、创建者、状态和打开链接，而不是退化成纯文本。
...
- **1.4.0** — 把逐用户企微授权收敛为设置页维护的一份部署级共享账号；加入解绑/重新绑定、会议状态跟踪、可配置 AI 总结和经过成员权限过滤的总结接口；让直接打开的分支 Session 复用群聊消息与输入组件；补齐分支主题消息的工具输出 schema；并让分支消息复用父群的人类优先自动回复判断流程。
